...
# out_dir = opt['out_dir']
def measure(out_dir, label_dir,is_psnr=True,is_ssim=False,flag=False):
    ...
    for img in imgs:
        ...
        if is_ssim:
            if flag:
                ssims.append(ssim(np.array(img),np.array(label)))
            else:
...
if __name__ == "__main__":
    # li = [file for file in Path('*').iterdir()]
    # for i in li:
    #     if i.is_dir():
    #         measure(str(i), '/data1/lzy/experiment/base/datasets/sots/testB',True,True,False)
    measure('output_images', 'Gt_images',True,True,True)
